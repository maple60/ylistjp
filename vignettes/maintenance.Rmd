...
| `academic_name()` behavior | `R/lookup.R`, `tests/testthat/test-lookup.R` |
| `ylist_search()` fields or matching rules | `R/lookup.R`, `tests/testthat/test-lookup.R` |
...
YList's public tab-delimited file is currently read with CP932 decoding. Keep
encoding tests in the synthetic fixture rather than downloading the full YList
...
inspectable; avoid silently changing `academic_name()` into a fuzzy lookup.
